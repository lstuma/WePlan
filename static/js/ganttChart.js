...
export function GanttChart(ganttChartElement, tasks, taskDurations) {
  ...
  const submit2save = contentFragment.querySelector("#submit-save");
  ...
  function handlesubmit2save() {
      fetch(`/save?tasks=${JSON.stringify(tasks)}&taskDurations=${JSON.stringify(taskDurations)}`);
  }
...
taskSelect.innerHTML = `
  ...
  submit2save.addEventListener("submit", handlesubmit2save);
